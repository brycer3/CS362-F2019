...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case baron:
        return cardBaron(choice1, state, currentPlayer);
    ...
    case minion:
        return cardMinion(choice1, state, handPos, currentPlayer);
    ...
    case ambassador:
        return cardAmbassador(choice1, choice2, state, handPos, currentPlayer);
...
int cardBaron(int choice1, struct gameState *state, int currentPlayer) {
    ...
    if (choice1 > 0) { //Boolean true or going to discard an estate
        ...
        while(1) { //Look for estate card in hand
            if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
                state->coins += 4;//Add 4 coins to the amount of coins
                state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][p];
                state->discardCount[currentPlayer]++;
                for (; p < state->handCount[currentPlayer]; p++) { //Shift cards over to eliminate estate card
...
int cardMinion(int choice1, struct gameState *state, int handPos, int currentPlayer) {
    //+1 action
    state->numActions++;
